...
namespace Application\ChatBundle\Cache\Engine;

use Application\ChatBundle\Cache\Cacheable;
...
class Mongo extends Base
{
    ...
    {
        $cache = $this->dm->getRepository('ChatBundle:CacheEntry')->getByKey($key);

        if (!$cache) {
            $cache = new \Application\ChatBundle\Document\CacheEntry();
            $cache->setKey($key);
